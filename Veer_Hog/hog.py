...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    def say(score0, score1, previous_score = last_score, biggest_diff = running_high):
        if who == 0:
            if (score0 - previous_score) > biggest_diff:
                biggest_diff = score0 - previous_score
                print(f"{biggest_diff} point(s)! That's a record gain for Player {who}!")
            previous_score = score0  
        else:
            if (score1 - previous_score) > biggest_diff:
                biggest_diff = score1 - previous_score
                print(f"{biggest_diff} point(s)! That's a record gain for Player {who}!")
            previous_score = score1  
        return announce_highest(who, previous_score, biggest_diff)
    return say
    # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    def runner(*args):
        ...
        while (counter < trials_count):
            total_val += original_function(*args)
            counter += 1
        return total_val/trials_count
    return runner
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    num_of_dice = 1
    best_dice_option = 1
    largest_averaged_score = 0
    current_score = 0
    averaged_dice_example= make_averaged(roll_dice, trials_count)
    while num_of_dice <= 10:
        current_score = averaged_dice_example(num_of_dice, dice)
        if current_score > largest_averaged_score:
            largest_averaged_score = current_score
            best_dice_option = num_of_dice
        num_of_dice += 1
    return best_dice_option
    # END PROBLEM 9
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    target_score = score + picky_piggy(opponent_score)
